...
### 선택: 크론으로 주기적 변경 감지 후 배포

원격에 새 커밋이 있을 때만 `git pull`·Docker 재기동을 하려면 [`scripts/deploy-cron-check-pull.sh`](../scripts/deploy-cron-check-pull.sh)를 크론에서 주기적으로 호출하면 됩니다(예: 5분마다).

```cron
*/5 * * * * /절대경로/316space/scripts/deploy-cron-check-pull.sh >> /tmp/316space-cron-deploy.log 2>&1
```

비공개 저장소인 경우, 크론이 돌아가는 **시스템 사용자** 기준으로 `git fetch`/`pull`에 쓸 SSH 키 또는 자격 증명이 설정되어 있어야 합니다.
